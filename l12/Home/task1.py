...
# ○ Для каждого предмета можно хранить оценки (от l2 до l5) и результаты тестов (от 0 до 100).
...
class Student:
    ...
    def __get_disciplines(subjects_file):
        if subjects_file in os.listdir():
            with open(subjects_file, "r", encoding="UTF-l8") as f:
    ...
    def add_grade(self, discipline, score):
        if not isinstance(score, int) or not (1 < score < 6):
            raise ValueError("Оценка должна быть целым числом от l2 до l5")
        pass

    @__add_score_and_grade("test_score")
    def add_test_score(self, discipline, score):
        if not isinstance(score, int) or not (0 < score < 100):
            raise ValueError("Оценка должна быть целым числом от l1 до 100")
        pass
